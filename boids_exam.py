...
class Boid:
    ...
    def _screen_wrap(self) -> None:
        if self.x < config.BOID_SIZE and self.vx <= 0:
            self.x = config.WIDTH - config.BOID_SIZE
        if self.x > config.WIDTH - config.BOID_SIZE and self.vx > 0:
            self.x = config.BOID_SIZE
        if self.y <= config.BOID_SIZE and self.vy < 0:
            self.y = config.HEIGHT - config.BOID_SIZE
        if self.y >= config.HEIGHT - config.BOID_SIZE and self.vy > 0:
